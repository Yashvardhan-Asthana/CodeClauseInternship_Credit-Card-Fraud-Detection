...
def train_model(X_train, y_train):
    ...
    return xgb_model, lr_model, rf_model

def create_voting_classifier(rf_model, xgb_model, lr_model):
...
def predict_transaction(models, accuracies, scaler, numeric_features, encoding_maps):
    ...
    prediction = models['Voting Classifier'].predict(input_df)[0]
    print("\nModel Prediction:", "Fraud" if prediction == 1 else "Non-Fraud")
...
if __name__ == "__main__":
    ...
    # Train models
    rf_model, xgb_model, lr_model = train_model(X_train, y_train)

    # Create and train Voting Classifier
    voting_clf = create_voting_classifier(rf_model, xgb_model, lr_model)
    voting_clf.fit(X_train, y_train)
    print("Voting Classifier training completed.")

    # Evaluate models
    models = {"Random Forest": rf_model, "XGBoost": xgb_model, "Logistic Regression": lr_model, "Voting Classifier": voting_clf}
